...
function checkLoginRateLimit(key) {
    ...
    const maxAttempts = LOGIN_RATE_LIMIT_MAX_ATTEMPTS;
...
async function ensureAffiliateSchema(env) {
    ...
    try {
        await db.prepare("ALTER TABLE order_items ADD COLUMN sold_by TEXT DEFAULT ''").run();
    } catch (error) {
        const message = String(error?.message || "");
        if (!/duplicate column|already exists/i.test(message)) throw error;
    }
...
async function readAllMarketplaceProducts(env) {
    await ensureMarketplaceSchema(env);
    const db = getCatalogDb(env);
    const rows = await getAllResults(db.prepare("SELECT id, data FROM marketplace_products").bind());
    const products = {};
    rows.forEach((row) => {
        try {
            products[row.id] = JSON.parse(row.data);
...
function truncateLines(lines, maxLength = 4096) {
...
function buildOrderLines(items, database) {
    ...
    const marketplaceProducts = database.marketplaceProducts || {};
    const lines = [];
...
async function getSavedOrderForNotification(env, record) {
    ...
    return {
        ...
        deliveryText: latestDelivery ? latestDelivery.delivery_text : "",
...
function formatProofLines(order) {
    const indent = "   ";
    if (order.paymentProof.type === "tt-card") {
        const codes = order.paymentProof.cardCodes.map((code, index) => `${indent}${index + 1}. ${telegramCode(code)}`);
        return [
            `${indent}${ICONS.ticket} Cards: ${order.paymentProof.cardCodes.length} x ${escapeHtml(formatTndAmount(order.paymentDetails.cardValue))}`,
            `${indent}Codes:`,
            ...codes,
        ];
    }
    ...
    return [
        `${indent}Proof: ${telegramCode(order.paymentProof.reference)}`,
    ];
}
...
function formatAdminMessage(order) {
    const products = order.lines
        .map((line) => {
            ...
            const seller = line.soldBy ? `\n  Sold by: ${escapeHtml(line.soldBy)}` : "";
            return `\u2022 ${escapeHtml(quantity)}${escapeHtml(productLabel)} \u2014 ${escapeHtml(formatTndAmount(line.lineTotal))}${seller}`;
        })
        .join("\n");

    const deliveryCodes = order.deliveryText
        ? order.deliveryText.split("\n").filter(Boolean).map((line, index) =>
              index === 0 ? `${ICONS.key} ${escapeHtml(line)}` : `   ${escapeHtml(line)}`
          )
        : [];
    ...
    return truncateLines([
        ...
        `\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501`,
        `${ICONS.phone} <code>+216 ${escapeHtml(formatTunisianPhone(order.customerPhone))}</code>`,
        ...(order.referredBy ? [`${ICONS.link} <code>${escapeHtml(order.referredBy)}</code>`] : []),
        "",
        ...
        `   Status: ${String(order.paymentStatus || "").toLowerCase() === "verified" ? ICONS.verify : ICONS.warning} ${escapeHtml(order.paymentStatusLabel)}`,
        ...(order.paymentStatusReason ? [`   Reason: ${escapeHtml(order.paymentStatusReason)}`] : []),
        "",
        `<b>Delivery:</b> ${String(order.deliveryStatus || "").toLowerCase() === "delivered" ? ICONS.verify : ICONS.delivery} ${escapeHtml(order.deliveryStatusLabel || getDeliveryStatusLabel(order.deliveryStatus || "waiting"))}`,
...
async function deleteTelegramMessage(env, chatId, messageId) {
    ...
    try {
        ...
        console.warn("Telegram message deletion failed", error);
    }
}

async function sendCustomerTelegramNotification(env, order, type) {
...
async function createAdminAffiliate(env, body) {
    try {
        ...
        console.error("[createAdminAffiliate] Error:", error);
...
async function handleAdminSaveData(body, request, env, corsHeaders) {
    ...
    if (body.marketplaceProducts && typeof body.marketplaceProducts === "object") {
        statements.push(db.prepare("DELETE FROM marketplace_products").bind());
        Object.entries(body.marketplaceProducts).forEach(([id, product]) => {
            statements.push(
                db
                    .prepare("INSERT INTO marketplace_products (id, data, created_at, updated_at) VALUES (?, ?, ?, ?)")
                    .bind(id, JSON.stringify(product), now, now),
            );
        });
    }
    ...
    console.log(`[admin-save] Successfully saved ${statements.length} statements (${body.categories?.length || 0} categories, ${body.products ? Object.keys(body.products).length : 0} products, ${body.marketplaceProducts ? Object.keys(body.marketplaceProducts).length : 0} marketplace products)`);
    return { ok: true };
}
...
async function handleAdminUploadImage(body, env) {
    ...
    const repo = env.GITHUB_REPO || "hyperkey.github.io"; // change repo name if forked
...
function parseTelegramAction(data) {
    ...
    if (target === "deliverycode") {
        ...
        if (!ORDER_ID_REGEX.test(orderId)) throw new Error("Unknown Telegram action");
        return {
            orderId,
            target,
        };
...
async function handleTelegramDeliveryReply(message, env) {
    ...
    const entry = pendingDeliveryOrders.get(String(chatId));
    if (!entry) return;

    const deliveryText = String(message.text || "").trim();
    if (!deliveryText) {
        await sendTelegramMessage(env, chatId, `${ICONS.warning} Send delivery details or 0.`);
        return;
    }

    pendingDeliveryOrders.delete(String(chatId));
    ...
    await saveAdminOrderDelivery(env, { orderId: entry.orderId, deliveryText });
    const record = await getOrderById(env, entry.orderId);
    if (record) {
        const order = await getSavedOrderForNotification(env, record);
        await editTelegramMessage(env, chatId, entry.messageId, formatAdminMessage(order), getAdminOrderKeyboard(order));
    }
}
...
async function handleTelegramWebhook(body, request, env, corsHeaders) {
    ...
    if (!callbackQuery) {
        if (body.message) {
            try {
                await handleTelegramDeliveryReply(body.message, env);
            } catch (error) {
    ...
    if (String(chatId || "") !== String(env.TELEGRAM_ADMIN_CHAT_ID || "")) {
        try {
            await answerTelegramCallback(env, callbackQuery.id, "This action is only available in the admin chat.", true);
        } catch (error) {
    ...
    try {
        ...
        try {
            await answerTelegramCallback(env, callbackQuery.id, "Unknown action", true);
        } catch (err) {
    ...
    if (action.target === "deliverycode") {
        try {
            ...
            await answerTelegramCallback(env, callbackQuery.id, `Send delivery details for ${action.orderId}`);
            await editTelegramMessage(
                env,
                chatId,
                messageId,
                [
                    `${formatAdminMessage(await getSavedOrderForNotification(env, record))}`,
                    "",
                    `${ICONS.key} Type delivery details below, or 0 to clear.`,
                ].join("\n"),
                undefined,
            );
        } catch (error) {
            try {
                await answerTelegramCallback(env, callbackQuery.id, error.message || "Could not start delivery flow", true);
            } catch (err) {
                console.warn("Telegram callback answer failed", err);
            }
        }
        ...
        if (chatId && messageId) {
            try {
                ...
                try {
                    ...
                    console.warn("Telegram fallback send also failed", sendError);
                }
            }
        }

        try {
            await sendCustomerTelegramNotification(env, order, action.target === "delivery" ? action.status : null);
        } catch (_) {}
    } catch (error) {
        try {
            await answerTelegramCallback(env, callbackQuery.id, error.message || "Could not update order", true);
        } catch (err) {
            console.warn("Telegram callback answer failed", err);
        }
    }
    ...
    if (existingSavedOrder) {
        if (!existingSavedOrder.telegram_notified_at) {
            try {
                ...
                console.warn("Telegram notification failed for existing order", error);
            }
        }
    ...
    if (savedOrder.duplicate) {
        if (savedOrder.record && !savedOrder.record.telegram_notified_at) {
            try {
                ...
                console.warn("Telegram notification failed for duplicate order", error);
            }
        }
